...
const CORE_PACKAGE_HANDLES = ['concrete5/core', 'concretecms/core'];

/**
 * @typedef {object} Result
 * @property {boolean} requiresComposerPkg
 */
...
function shouldRunComposer(data) {
  if (
    Object.keys(data.require || {}).some((dependency) => {
  ...
  if (
    Object.keys(data.autoload || {}).some((autoloadKey) => {
...
/**
 * @param {string} path
 * @returns {Promise<Result|null>} returns NULL if there's no composer.json file, or if running composer is not needed
 */
export default async function parseFile(path) {
  ...
  if (composerHasCoreDependency(data)) {
    ...
    result.requiresComposerPkg = false;
    console.log('We can use the plain composer command to install the Composer dependencies.');
  }
